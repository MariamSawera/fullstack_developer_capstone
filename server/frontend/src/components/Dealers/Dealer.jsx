...
const Dealer = () => {
  ...
  const get_dealer = async () => {
    ...
    if (retobj.status === 200) {
      let dealerData = retobj.dealer;
  
      // If dealer is an array (old API)
      if (Array.isArray(dealerData)) {
        setDealer(dealerData[0] || {});
      } 
      // If dealer is already a single object (new API)
      else if (dealerData && typeof dealerData === "object") {
        setDealer(dealerData);
      } 
      else {
        console.error("Invalid dealer data format");
        setDealer({});
      }
    }
  };
    ...
    <div style={{ margin: "20px" }}>
      ...
      <div style={{ marginTop: "10px" }}>
        {Object.keys(dealer).length === 0 ? (
          <div>Loading dealer details...</div>
        ) : (
          <>
            <h1 style={{color:"grey"}}>
